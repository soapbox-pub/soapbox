...
export const defaultMediaVisibility = (status: Status | undefined | null, displayMedia: string): boolean => {
  ...
  return (displayMedia !== 'hide_all' && !actualStatus.sensitive || displayMedia === 'show_all');
};

/** Grab the first external link from a status. */
export const getFirstExternalLink = (status: Status): HTMLAnchorElement | null => {
  try {
...
export const shouldHaveCard = (status: Status): boolean => {
...
export const hasIntegerMediaIds = (status: Status): boolean => {
  return status.media_attachments.some(({ id }) => isIntegerId(id));
};

/** Sanitize status text for use with screen readers. */
export const textForScreenReader = (intl: IntlShape, status: Status, rebloggedByText?: string): string => {
