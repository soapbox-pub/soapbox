...
import { relayLookupResultSchema } from 'soapbox/schemas/relay';
import { selectAccount } from 'soapbox/selectors';
import toast from 'soapbox/toast';
import { isLoggedIn } from 'soapbox/utils/auth';
import { getFeatures, parseVersion, PLEROMA } from 'soapbox/utils/features';
import { prettyList } from 'soapbox/utils/pretty-list';
...
const lookupAccountInRelay = (id: string, username: string, withNotes = false) =>
  (dispatch: AppDispatch, getState: () => RootState) => {
    if (!isLoggedIn(getState)) return null;

    return api(getState)
      .get(`/api/v1/admin/ditto/lookup/${id}${withNotes ? '?with_notes=true' : ''}`)
      .then(response => response.data)
      .then(result => relayLookupResultSchema.parse(result))
      .then(lookupResult => {
        const things: string[] = [];
        if (lookupResult.gotKind0) things.push('profile');
        if (lookupResult.gotKind3) things.push('follow list');
        if (lookupResult.notesCount) things.push('notes');
        if (things.length) {
          toast.success(`Got @${username}'s ${prettyList(things)}`);
          if (withNotes && !lookupResult.notesCount) {
            toast.error(`No notes found for @${username}`);
          }
        } else {
          toast.error(`No data found for @${username}`);
        }
      });
  };
