...
function useVerify() {
  ...
  async function verify(accountIds: string[], callbacks?: EntityCallbacks<void, unknown>) {
    ...
    try {
      await api.put('/api/v1/pleroma/admin/users/tag', { json: { nicknames: accts, tags: ['verified'] } });
      callbacks?.onSuccess?.();
    } catch (e) {
      callbacks?.onError?.(e);
  ...
  async function unverify(accountIds: string[], callbacks?: EntityCallbacks<void, unknown>) {
    ...
    verifyEffect(accountIds, false);
    try {
      await api.delete('/api/v1/pleroma/admin/users/tag', { json: { nicknames: accts, tags: ['verified'] } });
      callbacks?.onSuccess?.();
    } catch (e) {
      callbacks?.onError?.(e);
      verifyEffect(accountIds, true);
